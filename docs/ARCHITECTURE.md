...
Books processed before this exist unchanged; `tidy_existing_document` (`python manage.py tidy_book --document <id> [--dry-run]`) applies the same plan to one, leaving any module that student work refers to where it is, removing the folded modules' unattempted automatic quizzes and lessons with them, and queueing a new lesson and quiz for each module that received text.
