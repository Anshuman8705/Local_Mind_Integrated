...
import { META, clearAll, readEntry, writeEntry } from "@/offline/store";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const clear = useCallback(async () => {
    stopOfflineSync(); await clearAll();
  ...
  const adopt = useCallback(async (me: User) => {
    ...
    if (owner && owner !== me.id) await clearAll();
    ...
    (async () => {
      ...
      if (t) {
        try {
          ...
          if (saved) { setUser(saved); setMustChange(false); setSessionId(t.session_id ?? null); if (saved.role === "student") void startOfflineSync(); }
          else await clear();
        }
      }
      setReady(true);
